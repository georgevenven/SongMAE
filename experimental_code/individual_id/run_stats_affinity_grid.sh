...
run_one() {
  ...
  MPLBACKEND=Agg python experimental_code/individual_id/run_individual_id_umap.py \
    --encoder SongMAE \
    --species "$species" \
...
if [[ "$NO_COLLAGE" != "1" ]]; then
  python experimental_code/individual_id/plotting/make_stats_affinity_grid_collages.py --out_root "$OUT_ROOT"
fi
