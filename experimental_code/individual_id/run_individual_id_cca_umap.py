...
from src.core.utils import load_audio_params, load_json_events, majority_label, resolve_single_spec_path  # noqa: E402
from experimental_code.individual_id.run_individual_id_umap import (  # noqa: E402
...
def _write_recording_cluster_spectrograms(out_dir, rep_name, spec_dir, bird_labels, recording_labels, clusters):
    ...
    for recording in sorted(set(recording_labels.tolist())):
        ...
        if non_noise.size == 0:
            continue
        bird = majority_label(bird_labels[idx]) if np.issubdtype(bird_labels.dtype, np.integer) else str(np.unique(bird_labels[idx])[0])
        cluster = majority_label(non_noise)
        records.append(
            {
                "bird": str(bird),
