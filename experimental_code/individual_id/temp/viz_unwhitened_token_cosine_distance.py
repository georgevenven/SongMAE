...
def args_for(recording_stem):
    return {
        ...
        "npz_dir": str(OUT_DIR / f"{recording_stem}_xcl_5s_fp8_no_whiten.npz"),
        "json_path": None,
        "bird": None,
...
model_state = extract_embedding.load_model_state(args_for(SAMPLES[0]))
norm, stats_dir = extract_embedding.get_native_input_normalization(model_state)
for sample in SAMPLES:
    sample_args = args_for(sample)
