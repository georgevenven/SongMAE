...
all_features = np.concatenate([features for _, _, _, features in items], axis=0).astype(np.float32, copy=False)
mean = all_features.mean(axis=0, keepdims=True)
_, singular_values, vt = np.linalg.svd(all_features - mean, full_matrices=False)
bars_by_item = [((features - mean) @ vt[:1].T).T for _, _, _, features in items]
...
fig.suptitle(
    f"one global SVD, 5s, no whitening; SV1 singular value {singular_values[0]:.1f}",
    fontsize=12,
)
...
out_path = OUT_DIR / "xcm_canary_bf_zf_4x3_5s_one_global_svd_sv1_only_no_whiten.png"
