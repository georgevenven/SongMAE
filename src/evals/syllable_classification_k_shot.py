...
def k_shot_indices(y, spans, groups, train, k, seed):
    runs = []
    current = []
    ...
    for label, label_runs in sorted(by_label.items()):
        assert len(label_runs) >= k, f"Class {label} has {len(label_runs)} train occurrences, need {k}."
        for choice in sorted(rng.choice(len(label_runs), k, replace=False).tolist()):
            run = label_runs[choice]
            selected.extend(run)
            occurrences.append(
                {
                    "label": label,
    ...
    return np.array(sorted(selected), dtype=np.int64), occurrences


def manifest_indices(y, spans, groups, train, path, k):
    manifest = json.loads(Path(path).read_text())
    assert manifest["shots_per_class"] == k
    occurrences = manifest["selected_occurrences"]
    local = k_shot_indices(y, spans, groups, train, k, manifest["seed"])[1]
    background = iter(row for row in local if row["label"] == 0)
    selected = []
    matched = []
    assigned = {}
    for occurrence in occurrences:
        overlapping = [
            index
            for index in np.flatnonzero(train)
            if groups[index] == occurrence["group"]
            and max(spans[index][1], occurrence["start_ms"])
            < min(spans[index][2], occurrence["end_ms"])
        ]
        matches = [index for index in overlapping if int(y[index]) == occurrence["label"]]
        if not matches:
            if occurrence["label"]:
                matches = overlapping
            else:
                occurrence = next(background)
                matches = [
                    index
                    for index in np.flatnonzero(train)
                    if groups[index] == occurrence["group"]
        ...
        assert matches, occurrence
        for index in matches:
            assert assigned.get(index, occurrence["label"]) == occurrence["label"]
            assigned[index] = occurrence["label"]
            y[index] = occurrence["label"]
        selected.extend(matches)
        matched.append(occurrence)
    return np.array(sorted(set(selected)), dtype=np.int64), matched
...
def standardized_batch(x, indices, mean, std):
    batch = np.asarray(x[indices], dtype=np.float32)
    batch -= mean
    batch /= std
    return batch
...
def pca_features(x, indices, components, seed):
    if not components:
        return x
    assert components <= min(indices.size, x.shape[1]), (indices.size, x.shape)
    model = PCA(n_components=components, svd_solver="randomized", random_state=seed)
    model.fit(np.asarray(x[indices], dtype=np.float32))
    return model.transform(x).astype(np.float32, copy=False)


def fit_logreg_classifier(x, y, indices, batch_size):
    mean, std = feature_stats(x, indices, batch_size)
    model = LogisticRegression(C=1.0, max_iter=1000).fit(
        standardized_batch(x, indices, mean, std), y[indices]
    )
    return model, mean, std


def predict_logreg_classifier(model, x, indices, batch_size, mean, std):
    predictions = []
    for start in range(0, indices.size, batch_size):
        batch = standardized_batch(x, indices[start : start + batch_size], mean, std)
...
def fit_torch_classifier(x, y, indices, args):
    ...
    mean, std = feature_stats(x, indices, args.batch_size) if args.standardize else (0.0, 1.0)
    model.train()
...
def parse_args():
    ...
    parser.add_argument("--model", choices=["logreg", "linear", "mlp", "scalar_mix"], default="linear")
    ...
    parser.add_argument("--shots_json")
...
def main():
    ...
    assert budget is None, "K-shot evaluation does not use --max_train_seconds."
    assert args.shots_per_class > 0
    assert args.standardize or args.model != "logreg"
    ...
    if args.shots_json:
        train_indices, occurrences = manifest_indices(
            y, spans, groups, train, args.shots_json, args.shots_per_class
        )
    else:
        train_indices, occurrences = k_shot_indices(
            y, spans, groups, train, args.shots_per_class, args.seed
        )
    assert len(set(y[train_indices].tolist())) >= 2, "Train split has fewer than two classes."
    ...
    train_groups = sorted(set(groups[index] for index in train_indices))
    train_seconds = sum(spans[index][2] - spans[index][1] for index in train_indices) / 1000
    assert not args.pca_components or args.model != "scalar_mix"
    x = pca_features(x, train_indices, args.pca_components, args.seed)

    if args.model == "logreg":
        model, mean, std = fit_logreg_classifier(x, y, train_indices, args.batch_size)
        pred = predict_logreg_classifier(model, x, val_indices, args.batch_size, mean, std)
    else:
    ...
    metrics.update(
        {
            ...
            "train_occurrences": len(occurrences),
