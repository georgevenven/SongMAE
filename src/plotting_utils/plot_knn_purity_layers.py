#!/usr/bin/env python3
"""Plot four-model kNN purity across encoder layers."""
import argparse
import json
...
def load_purity(root, k):
    ...
    for path in root.rglob("summary.json"):
        _, _, model, layer_name = path.relative_to(root).parts[:4]
        row = next(row for row in json.loads(path.read_text())["rows"] if row["k"] == k)
        values[model, int(layer_name.removeprefix("layer_"))].append(
            100 * row["macro_same_purity"]
        )
    means = {key: sum(rows) / len(rows) for key, rows in values.items()}
    assert all(len(values[model, layer]) == 50 for model in MODELS for layer in LAYERS)
    return means
...
def main():
    ...
    for axis, (model, (label, color)) in zip(axes, MODELS.items()):
        ...
        axis.plot(LAYERS, purity, color=color, marker="o", markersize=4, linewidth=2.25)
    ...
    output = output_dir / f"layer_purity_k{args.k}.png"
