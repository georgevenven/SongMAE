...
def draw_origin_axes(ax):
    ...
    for end in axes:
        ...
        ax.quiver(
            ...
            color=(0, 0, 0, 0.82),
            linewidth=1.05,
            arrow_length_ratio=0.72,
            normalize=False,
        )


def draw_path_arrow(ax, path, color):
    start_index = path.shape[0] - 8
    end_index = path.shape[0] - 1
    start = path[start_index]
    delta = path[end_index] - start
    ax.quiver(
        start[0],
        start[1],
        start[2],
...
def plot_latent_space(out_base):
    ...
    for path, color in zip(paths, COLORS):
        ...
        nodes = path[:: max(1, path.shape[0] // 14)]
        ax.scatter(
            nodes[:, 0],
            nodes[:, 1],
            nodes[:, 2],
