...
from individual_id.latent_trajectory_decoder.train_t2vec_gru_tokens import T2VecTokenGRU
...
def nearest_tokens(features, centers):
...
def load_model(path, input_dim, device):
    ...
    codebook = np.load(Path(path).parent / "kmeans_codebook.npz")
    centers = codebook["centers"].astype(np.float32)
    assert centers.shape[1] == input_dim
    model = T2VecTokenGRU(
        int(args["vocab_size"]),
        int(args["hidden_dim"]),
        int(args["layers"]),
    ...
    model.eval()
    return model, args, centers


@torch.no_grad()
def model_states(model, tokens, device):
    clean = torch.from_numpy(tokens).to(device=device, dtype=torch.long)[None]
    lengths = torch.tensor([tokens.shape[0]], device=device, dtype=torch.long)
    _, h, state = model(clean, lengths, clean)
    return h.squeeze(0).cpu().numpy().astype(np.float32), state.squeeze(0).cpu().numpy().astype(np.float32)


def pooled_states(rows, model, centers, args, device):
    x_parts = []
    bird_parts = []
    syllable_parts = []
    recording_parts = []
    ...
    for index, (recording, bird, seq, syllables) in enumerate(rows):
        tokens = nearest_tokens(seq, centers)
        h, state = model_states(model, tokens, device)
...
def parse_args():
    parser = argparse.ArgumentParser(description="UMAP t2vec token-GRU hidden states.")
...
def main():
    ...
    model, model_args, centers = load_model(args.model_pt, input_dim, device)
    features, bird_labels, syllable_labels, recording_labels, states, state_birds, state_recordings = pooled_states(rows, model, centers, args, device)

    rep_name = f"t2vec_token_gru_decoder_state_pool_{args.pool_mode}_sliding_w{args.pool_window}_h{args.pool_hop}_raw"
    ...
    _scatter_umap(state_xy, state_birds, f"{args.species_display_name}: t2vec token encoder states", out_dir / "t2vec_token_encoder_recording_state_raw")
