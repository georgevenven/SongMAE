...
from individual_id.latent_trajectory_decoder.train_t2vec_gpt_latents import BottleneckGPT
from individual_id.latent_trajectory_decoder.umap_t2vec_gru_latents import load_rows
...
def parse_args():
    parser = argparse.ArgumentParser(description="UMAP bottlenecked Transformer/GPT latent states.")
...
def main():
    ...
    rep_name = f"t2vec_gpt_decoder_state_pool_{args.pool_mode}_sliding_w{args.pool_window}_h{args.pool_hop}_raw"
    print(f"[umap] {rep_name}: points={features.shape[0]} dim={features.shape[1]}")
    xy = _fit_umap(features, args.umap_neighbors, args.umap_min_dist, args.umap_metric, args.seed, args.umap_negative_sample_rate)
    plot_args = Namespace(species_display_name=args.species_display_name, hdbscan_min_cluster_size=args.hdbscan_min_cluster_size, hdbscan_min_samples=args.hdbscan_min_samples)
    ...
    state_xy = _fit_umap(states, min(15, max(2, states.shape[0] - 1)), args.umap_min_dist, args.umap_metric, args.seed, args.umap_negative_sample_rate)
    _scatter_umap(state_xy, state_birds, f"{args.species_display_name}: bottleneck states", out_dir / "t2vec_gpt_bottleneck_recording_state_raw")
