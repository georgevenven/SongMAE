...
MODELS="${MODELS:-xcl_base_100k_p16x4_c010 xcl_base_100k_p32x1_c010 birdaves_biox_base hubert_base_ls960}"
...
for row in "${DATASETS[@]}"; do
  ...
  while IFS= read -r bird; do
    for model in "${MODEL_LIST[@]}"; do
      ...
      run_knn "$model" "$out_dir" "$json" "$spec_dir" "$wav_dir" "$bird" "$LAYER" "$PCA_COMPONENTS" "$TARGET_FEATURE_TYPE" | tee -a "$OUT_ROOT/run.log"
